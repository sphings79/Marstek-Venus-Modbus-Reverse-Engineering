...
## Vollständige Analyse-Dokumentation — Venus D (VNSD-0) v1 (Juli 2026)
...
> `VNSD-0_app_1492_0702_142136.bin` (149.2) zusammen. Die Abschnitte 1–16 und die
> Anhänge darunter enthalten die **vollständige akkumulierte RE-Analyse** des Geräts
> (Protokoll, Register-Map, Local API, Cloud/OTA, Local-Update-Verfahren, Skripte,
> Methodik). Wo firmware-spezifische Adressen/Zahlen genannt werden, gilt für 149.2
...
Ghidra:     ARM:LE:32:Cortex, ~1600 Funktionen
```
...
Tabellen-Ptr:      DAT_0801EC30 (=0x20000354); 2. Reader-Referenz @0x08029F50
...
`+10` Scale · `+11` Count. Typ-Codes: `01`=u8 `02`=u16 `04`=u32, high-nibble 1 = signed
(`11`=i8 `12`=i16 `14`=i32), `24`=float. Scale-Codes: `0`=×1 `1`=×0.1 `2`=×0.01 `3`=×10 `4`=×100
(Konstanten IEEE-754 @ `0x0805009C`/`A0`/`A4`).
...
Die 246 Registernummern sind aus dem Flash **nicht statisch enumerierbar** (vier unabhängige
lokale PyGhidra-Prüfungen): (1) kein aufsteigendes Register-Array im Flash (nur False-Positive
`0x08036C10`), (2) keine dichten Register-`MOVW`-Immediates (Spitzenreiter Write-Handler
`FUN_08050f20`, nur 27 Konstanten), (3) nur **2** Code-Referenzen auf die Basis `0x20000354` —
beide Reader, **kein** Builder, (4) kein `.data`-Template (Pointer-Signatur @+2/+4/+6/+8 in
CCM+SRAM negativ; `_sidata+0x354` negativ). ⇒ Registernummern werden im Init-Code **berechnet**
(z. B. 34000 + Pack·100 + Feld-Offset). Enumeration nur per **Live-Scan** oder **Emulation**.
...
| `micro` | Inverter-Co-Prozessor (`vd_inv_app`) | `vd_inv_app_0116_0702_ota_163439.bin` (116) | ⏳ offen |

Der `micro`-Baustein ist ein **separater** Wechselrichter-Controller neben dem Control-MCU;
eigene Analyse (Basis, IVT, Register/Protokolle) steht aus.
...
| `0x0801e43c` | `FUN_0801e43c` | **Modbus Dispatcher** (149.2) |
| `0x0801eaa4` | `FUN_0801eaa4` | **FC03 Read-Handler** (149.2) |
| `0x0804fe20` | `FUN_0804fe20` | **Read-Serializer** (149.2) |
| `0x08050f20` | `FUN_08050f20` | **Write-Handler** (149.2) |
...
### 16.3 Descriptor-Tabelle (246 Einträge) — noch nicht extrahiert
...
Aus dem Descriptor-Table-Schema (246 Einträge) und 413 gescannten Registern
